<template>
  <div>
    <el-form :model="datasourceForm" :rules="rules" ref="datasourceForm">
      ...
      <el-form-item label="Saiku Server (http://domain:port)" prop="server">
          <el-input v-model="datasourceForm.server"></el-input>
      </el-form-item>
      <el-form-item label="User Name (for Saiku Server)" prop="username">
  ...
  data() {
    return {
      ...
      rules: {
        ...
        server: [
          { required: true, message: '请输入server', trigger: 'blur' }
        ],
        username: [
